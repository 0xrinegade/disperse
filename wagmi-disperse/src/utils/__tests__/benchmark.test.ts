...
import { isDisperseContract, bytecodeCache } from "../contractVerify";
...
describe("Performance Benchmarks", () => {
  ...
  describe("Contract verification performance", () => {
    ...
    it("should cache contract verification results efficiently", () => {
      ...
      console.log(`Contract verification - First call: ${firstCallTime.toFixed(3)}ms, Cached call: ${secondCallTime.toFixed(3)}ms`);
    });
  ...
  describe("Recipient parsing performance", () => {
    it("should parse recipients efficiently", () => {
      const recipientData = Array.from({ length: 100 }, (_, i) => 
        `0x${'1'.repeat(40)} ${(i + 1).toString()}`
      ).join('\n');
    ...
    it("should handle large recipient lists efficiently", () => {
      const recipientData = Array.from({ length: 1000 }, (_, i) => 
        `0x${'a'.repeat(40)} ${(i + 1).toString()}.5`
      ).join('\n');
